...
public class Test {
    ...
    public static CPLangParser readFromFile(String filename) throws IOException {
    ...
    public static void task12() throws IOException {
         ...
         */
        var parser = readFromFile("inputs/manual.txt");

        /* DONE1 înlocuiește expr cu regula de start din gramatică */
    ...
    public static CPLangParserVisitor<Integer> task3setup() {
        ...
        return new CPLangParserBaseVisitor<Integer>() {
            ...
            /*
             * DONE3 Suprascrie visit<eticheta> unde eticheta corespunde
             * operațiilor aritmetice, operațiilor parantezate, minus-unar și
             * literalilor întregi
             */
            @Override
            public Integer visitUnary(CPLangParser.UnaryContext ctx) {
                return -visit(ctx.expr());
            }

            @Override
            public Integer visitMultDiv(CPLangParser.MultDivContext ctx) {
            ...
            @Override
            public Integer visitPlusMinus(CPLangParser.PlusMinusContext ctx) {
                if (ctx.op.getText().equals("+"))
                    return visit(ctx.lhs) + visit(ctx.rhs);
                else
                    return visit(ctx.lhs) - visit(ctx.rhs);
            ...
            @Override
            public Integer visitGuaredExpr(CPLangParser.GuaredExprContext ctx) {
                return visit(ctx.expr());
            }
        };
    }
    ...
    public static void task3() throws IOException {
         ...
         **/
        var parser = readFromFile("inputs/input3.txt");

        /* 3 înlocuiește expr cu regula de start */
        var tree = parser.prog();
